...
## User Story
As backend systems, we want destination-based outbox retries so downstream delivery is reliable.

## Story Intent
This story exists to deliver a concrete capability in the AI STR OS command center: As backend systems, we want destination-based outbox retries so downstream delivery is reliable. The expected business impact is that the intended business outcome is achieved.
