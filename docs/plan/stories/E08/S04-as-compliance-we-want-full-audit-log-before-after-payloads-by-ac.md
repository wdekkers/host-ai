...
As compliance, we want full audit log before/after payloads by actor so critical decisions are reviewable.

## Story Intent

This story exists to deliver a concrete capability in the AI STR OS command center: As compliance, we want full audit log before/after payloads by actor so critical decisions are reviewable. The expected business impact is that the intended business outcome is achieved.
