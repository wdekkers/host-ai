...
## User Story
As operations users, we want approval queue and property state projections so UI views are fast and task-focused.

## Story Intent
This story exists to deliver a concrete capability in the AI STR OS command center: As operations users, we want approval queue and property state projections so UI views are fast and task-focused. The expected business impact is that the intended business outcome is achieved.
