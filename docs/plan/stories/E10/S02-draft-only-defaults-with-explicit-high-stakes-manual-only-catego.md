...
**Story ID:** `E10-S02`  
**Status:** Draft  
**Primary Actor:** host safety

## User Story

As host safety, I want draft-only defaults with explicit high-stakes manual-only categories so critical outcomes remain human-controlled.

## Story Intent

This story exists to deliver a concrete capability in the AI STR OS command center: draft-only defaults with explicit high-stakes manual-only categories. The expected business impact is that critical outcomes remain human-controlled.
