...
As the AI team, we want to capture host edits as training signals so response quality improves with real usage.

## Story Intent

This story exists to deliver a concrete capability in the AI STR OS command center: As the AI team, we want to capture host edits as training signals so response quality improves with real usage. The expected business impact is that the intended business outcome is achieved.
