...
As the internal team, we want to deploy first across 3 STR + 1 MTR properties so we can validate reliability before external launch.

## Story Intent

This story exists to deliver a concrete capability in the AI STR OS command center: As the internal team, we want to deploy first across 3 STR + 1 MTR properties so we can validate reliability before external launch. The expected business impact is that the intended business outcome is achieved.
