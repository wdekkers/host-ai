...
import { createPropertyQaEntryInSingleton, listPropertyQaEntriesInSingleton } from '@/lib/command-center-store';
...
export const POST = withPermission('drafts.write', async (request: Request, { params }: Params, authContext) => {
  const { propertyId } = await params;
  try {
    const rawBody = (await request.json()) as { createdBy?: unknown };
    const parsed = createPropertyQaEntryInputSchema.parse(rawBody);
    const createdBy =
      process.env.NODE_ENV !== 'production' && typeof rawBody.createdBy === 'string' && rawBody.createdBy.length > 0
        ? rawBody.createdBy
        : authContext.userId;
