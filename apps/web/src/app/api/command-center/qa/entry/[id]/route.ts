...
export const PATCH = withPermission('drafts.write', async (request: Request, { params }: Params, authContext) => {
  const { id } = await params;
  try {
    const rawBody = (await request.json()) as { updatedBy?: unknown };
    const parsed = updatePropertyQaEntryInputSchema.parse(rawBody);
    const updatedBy =
      process.env.NODE_ENV !== 'production' && typeof rawBody.updatedBy === 'string' && rawBody.updatedBy.length > 0
        ? rawBody.updatedBy
        : authContext.userId;
