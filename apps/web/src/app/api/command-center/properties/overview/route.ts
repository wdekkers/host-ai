...
export async function GET() {
  ...
  const propertyIds = Array.from(
    new Set(queue.map((item) => propertyIdFromReservation(item.reservationId))),
  );

  const items = propertyIds.map((propertyId) => {
    const propertyQueue = queue.filter(
      (entry) => propertyIdFromReservation(entry.reservationId) === propertyId,
    );
    const pendingCount = propertyQueue.filter(
  ...
  return NextResponse.json({
    items: items.sort((left, right) => {
      const readinessRank = (value: string) =>
        value === 'blocked' ? 2 : value === 'at-risk' ? 1 : 0;
      return (
