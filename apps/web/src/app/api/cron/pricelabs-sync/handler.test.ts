...
void describe('handleCronSync', () => {
  ...
  void it('maps skipped_no_credentials to skipped status', async () => {
    const res = await handleCronSync(authed(), {
      cronSecret: 'test-secret',
      getOrgsWithCredentials: async () => [{ orgId: 'org-1' }],
      runForOrg: async () => ({ status: 'skipped_no_credentials' } as const satisfies SyncResult),
    });
    const body = await res.json() as { results: { orgId: string; status: string }[] };
