...
export function MessageThread({
  ...
  useEffect(() => { stateRef.current = { msgs, hasMore, loading }; }, [msgs, hasMore, loading]);
  ...
  useLayoutEffect(() => {
    if (prevScrollInfoRef.current) {
      ...
      window.scrollTo({ top: scrollY + (document.body.scrollHeight - scrollHeight), behavior: 'instant' as ScrollBehavior });
      prevScrollInfoRef.current = null;
    }
  }, [msgs]);

  async function loadMore() {
    const { msgs: currentMsgs, hasMore: currentHasMore, loading: currentLoading } = stateRef.current;
    if (currentLoading || !currentHasMore || currentMsgs.length === 0) return;

    setLoading(true);
    prevScrollInfoRef.current = { scrollY: window.scrollY, scrollHeight: document.body.scrollHeight };
