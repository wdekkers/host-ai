...
void test('maps route + method to permission', () => {
  ...
  assert.equal(getPermissionForApiRoute('/api/command-center/qa/property:abc', 'POST'), 'drafts.write');
  assert.equal(getPermissionForApiRoute('/api/command-center/qa-suggestions/sug-1/approve', 'POST'), 'drafts.write');
