...
import { GET as getOperatingProfile, PATCH as patchOperatingProfile } from '@/app/api/command-center/operating-profile/route';
import { GET as getAutopilotActions, POST as postAutopilotEvaluate } from '@/app/api/command-center/autopilot/route';
...
import { GET as getPropertyBrainProfile, PATCH as patchPropertyBrainProfile } from '@/app/api/command-center/property-brain/[id]/route';
import { POST as postPropertyBrainResolve } from '@/app/api/command-center/property-brain/resolve/route';
import { GET as getIntentTaxonomy, POST as postIntentDraft } from '@/app/api/command-center/intent-drafts/route';
...
import { GET as getCleanerJitPings, POST as postCleanerJitPing } from '@/app/api/command-center/cleaner-jit/pings/route';
import { PATCH as patchCleanerJitPing } from '@/app/api/command-center/cleaner-jit/pings/[id]/route';
import { GET as getMonitoringAlerts, POST as postMonitoringRun } from '@/app/api/command-center/monitoring/route';
...
import { GET as getTwilioThreads, POST as postTwilioThread } from '@/app/api/integrations/twilio/threads/route';
...
void test('allows host to reject a draft with audit trace', async () => {
  ...
  assert.equal(
    rejectedBody.item.auditLog.some((entry) => entry.action === 'rejected' && entry.actorId === 'host-reviewer'),
    true
  );
});
...
void test('blocks editing a sent draft via API with actionable error', async () => {
  ...
  const editSent = await updateDraftRoute(
    new Request(`http://localhost/api/command-center/queue/${createdBody.item.id}`, {
      method: 'PATCH',
      headers: { 'content-type': 'application/json' },
      body: JSON.stringify({ action: 'edit', actorId: 'host-reviewer', body: 'Trying to edit sent draft' })
    }),
    { params: Promise.resolve({ id: createdBody.item.id }) }
  );
...
void test('rejects invalid audit timeline query values with actionable errors', async () => {
  const invalidAction = await getAuditTimeline(new Request('http://localhost/api/command-center/audit?action=unknown'));
  ...
  const invalidSince = await getAuditTimeline(new Request('http://localhost/api/command-center/audit?since=not-a-date'));
  ...
  const invalidRange = await getAuditTimeline(
    new Request('http://localhost/api/command-center/audit?since=2026-03-01T00:00:00.000Z&until=2026-02-01T00:00:00.000Z')
  );
  const invalidRangeBody = (await invalidRange.json()) as { error?: string };
  assert.equal(invalidRange.status, 400);
  assert.equal((invalidRangeBody.error ?? '').includes('since must be earlier than or equal to until'), true);
});
...
void test('deduplicates Hospitable webhook event id to prevent duplicate queue items', async () => {
  ...
  assert.equal(listQueue().filter((item) => item.reservationId === payload.reservationId).length, 1);
});
...
void test('assembles per-property context from reservation, policy, and knowledge sources', async () => {
  ...
  const contextBody = (await contextResponse.json()) as {
    context: {
      ...
      policy: string;
      knowledgeSources: Array<{ type: string; snippet: string; confidence?: string; referenceUrl?: string }>;
...
void test('creates ai draft from inbound message using assembled context and source references', async () => {
  ...
  const draftedBody = (await drafted.json()) as { item: { body: string; sources: Array<{ type: string }> } };

  assert.equal(drafted.status, 200);
  assert.equal(draftedBody.item.body.toLowerCase().includes('jordan'), true);
  assert.equal(draftedBody.item.sources.some((source) => source.type === 'policy'), true);
  assert.equal(draftedBody.item.sources.some((source) => source.type === 'property-note'), true);
});
...
void test('captures before/after audit payloads for draft edits and approval transitions', () => {
  ...
  assert.equal(Boolean(approvedEntry && 'before' in approvedEntry && 'after' in approvedEntry), true);
});
...
void test('filters queue by intent via API query param', async () => {
  ...
  const response = await getQueueRoute(new Request('http://localhost/api/command-center/queue?intent=wifi-help'));
  const body = (await response.json()) as { items: Array<{ intent: string }> };
  assert.equal(body.items.length > 0, true);
  assert.equal(body.items.every((item) => item.intent === 'wifi-help'), true);
});
...
void test('filters queue by risk via API query param', async () => {
  ...
  const response = await getQueueRoute(new Request('http://localhost/api/command-center/queue?risk=high'));
  const body = (await response.json()) as { items: Array<{ body: string; intent: string }> };
  assert.equal(body.items.length > 0, true);
  assert.equal(
    body.items.every((item) => getRiskTrustIndicator({ intent: item.intent, body: item.body }).risk === 'high'),
    true
  );
});
...
void test('filters queue by trust via API query param', async () => {
  ...
  const response = await getQueueRoute(new Request('http://localhost/api/command-center/queue?trust=low'));
  const body = (await response.json()) as { items: Array<{ body: string; intent: string }> };
  assert.equal(body.items.length > 0, true);
  assert.equal(
    body.items.every((item) => getRiskTrustIndicator({ intent: item.intent, body: item.body }).trust === 'low'),
    true
  );
});
...
void test('returns draft history timeline for selected draft id', async () => {
  ...
  await updateDraftRoute(
    new Request(`http://localhost/api/command-center/queue/${createdBody.item.id}`, {
      method: 'PATCH',
      headers: { 'content-type': 'application/json' },
      body: JSON.stringify({ action: 'edit', actorId: 'history-editor', body: 'History edit body.' })
    }),
    { params: Promise.resolve({ id: createdBody.item.id }) }
  );
  await updateDraftRoute(
  ...
  const body = (await response.json()) as {
    items: Array<{ draftId: string; action: string; actorId: string; before?: Record<string, unknown> }>;
  };

  assert.equal(body.items.length >= 3, true);
  assert.equal(body.items.every((item) => item.draftId === createdBody.item.id), true);
  assert.equal(
...
void test('returns today priorities panel data focused on actionable queue items', async () => {
  ...
  const response = await getTodayPriorities(new Request('http://localhost/api/command-center/priorities?limit=3'));
  const body = (await response.json()) as {
    items: Array<{ reservationId: string; status: string; risk: string; priority: string; recommendedAction: string }>;
  };

  assert.equal(response.status, 200);
  assert.equal(body.items.length > 0, true);
  assert.equal(body.items.length <= 3, true);
  assert.equal(body.items.every((item) => item.status !== 'sent' && item.status !== 'rejected'), true);
  assert.equal(
    body.items.some(
      (item) =>
...
void test('rejects invalid priorities limit query', async () => {
  const response = await getTodayPriorities(new Request('http://localhost/api/command-center/priorities?limit=0'));
  const body = (await response.json()) as { error?: string };
...
void test('returns proactive communication suggestions for command center operators', async () => {
  ...
  assert.equal(body.items.every((item) => item.suggestedMessage.length > 0 && item.reason.length > 0), true);
  assert.equal(
    body.items.some(
      (item) =>
...
void test('supports cleaner JIT ping lifecycle with structured READY/ETA/NOT_READY responses', async () => {
  ...
  const listed = await getCleanerJitPings(
    new Request('http://localhost/api/command-center/cleaner-jit/pings?reservationId=res-cleaner-jit-001')
  );
  const listedBody = (await listed.json()) as { items: Array<{ reservationId: string; status: string }> };
  assert.equal(listed.status, 200);
  assert.equal(
    listedBody.items.some((item) => item.reservationId === 'res-cleaner-jit-001' && item.status === 'ETA'),
    true
  );
});
...
void test('runs monitoring agents and returns actionable operations alerts', async () => {
  ...
  assert.equal(
    runBody.items.some(
      (item) => item.category === 'missing-confirmation' && item.severity === 'high' && item.reservationId === 'res-monitoring-risky'
    ),
    true
  );

  const listResponse = await getMonitoringAlerts(new Request('http://localhost/api/command-center/monitoring?severity=high'));
  const listBody = (await listResponse.json()) as { items: Array<{ severity: string }> };
  assert.equal(listResponse.status, 200);
  assert.equal(listBody.items.length > 0, true);
  assert.equal(listBody.items.every((item) => item.severity === 'high'), true);
});
...
void test('computes property state readiness and blockers from monitoring signals', async () => {
  ...
  const body = (await response.json()) as {
    state: { propertyId: string; readiness: string; blockers: string[]; signals: { openAlerts: number } };
  };
...
void test('rejects invalid monitoring query filters', async () => {
  const response = await getMonitoringAlerts(new Request('http://localhost/api/command-center/monitoring?status=nope'));
  const body = (await response.json()) as { error?: string };
...
void test('provides immediate host alerts and drafted guest reassurance for active incidents', async () => {
  ...
  const response = await getIncidentAlerts(new Request('http://localhost/api/command-center/incidents/alerts'));
  const body = (await response.json()) as {
  ...
  assert.equal(response.status, 200);
  assert.equal(body.items.length > 0, true);
  assert.equal(body.items.every((item) => item.hostAlert.length > 0 && item.guestDraft.length > 0), true);
  assert.equal(
    body.items.some(
      (item) => item.urgency === 'immediate' && item.hostAlert.toLowerCase().includes('incident') && item.guestDraft.includes('Hi')
    ),
    true
  );
});
...
void test('scores experience risk by fix-impact and guest-sensitivity with separate urgency and compensation guidance', async () => {
  ...
  assert.equal(severeBody.compensationGuidance === 'consider-credit' || severeBody.compensationGuidance === 'escalate-review', true);
  ...
  assert.equal(lowBody.communicationUrgency === 'routine' || lowBody.communicationUrgency === 'same-day', true);
...
void test('returns portfolio trends for incidents refunds amenity reliability and review quality', async () => {
  const response = await getPortfolioTrends(new Request('http://localhost/api/command-center/portfolio-trends'));
  const body = (await response.json()) as {
    trends: {
...
void test('evaluates selective autopilot only for safe intents and logs actions', async () => {
  ...
  const actions = await getAutopilotActions(new Request('http://localhost/api/command-center/autopilot'));
  const actionsBody = (await actions.json()) as { items: Array<{ intent: string; status: string }> };
  assert.equal(actions.status, 200);
  assert.equal(actionsBody.items.length > 0, true);
  assert.equal(actionsBody.items.some((item) => item.intent === 'wifi-help' && item.status === 'executed'), true);
});
...
void test('supports autopilot rollback with full action logging', async () => {
  ...
  const rolledBackBody = (await rolledBack.json()) as { item: { status: string; rollbackReason?: string } };
...
void test('returns and updates operating profile with per-property risk tolerances', async () => {
  const before = await getOperatingProfile(new Request('http://localhost/api/command-center/operating-profile'));
  const beforeBody = (await before.json()) as {
    profile: { strictness: number; generosity: number; propertyRiskTolerance: Record<string, number> };
  };
  ...
  assert.equal(beforeBody.profile.strictness !== updatedBody.profile.strictness || beforeBody.profile.generosity !== updatedBody.profile.generosity, true);
});
...
void test('returns append-only event log and outbox records for delivery tracking', async () => {
  ...
  const eventsResponse = await getEventBackbone(new Request('http://localhost/api/command-center/events?limit=10'));
  ...
  const outboxResponse = await getOutbox(new Request('http://localhost/api/command-center/outbox?limit=10'));
...
void test('retries outbox by destination and records attempts', async () => {
  ...
  assert.equal(retryBody.items.every((item) => item.destination === 'projection-updater'), true);
  assert.equal(retryBody.items.every((item) => item.attempts >= 1), true);
});
...
void test('supports full audit export with before-after payloads by actor', async () => {
  ...
  await updateDraftRoute(
    new Request(`http://localhost/api/command-center/queue/${createdBody.item.id}`, {
      method: 'PATCH',
      headers: { 'content-type': 'application/json' },
      body: JSON.stringify({ action: 'edit', actorId: 'audit-export-user', body: 'Updated for export.' })
    }),
    { params: Promise.resolve({ id: createdBody.item.id }) }
  );
...
void test('returns normalized entities for properties guests reservations and messages', async () => {
  ...
  const response = await getEntities(new Request('http://localhost/api/command-center/entities?kind=all'));
  const body = (await response.json()) as {
    entities: {
...
void test('captures property onboarding rules for check-in-out occupancy quiet-hours and house rules', async () => {
  ...
  const updated = await patchPropertyBrainProfile(
    new Request(`http://localhost/api/command-center/property-brain/${encodeURIComponent(propertyId)}`, {
      method: 'PATCH',
      headers: { 'content-type': 'application/json' },
      body: JSON.stringify({
        actorId: 'host-owner',
  ...
  const updatedBody = (await updated.json()) as {
    profile: { coreRules: { checkInTime: string; maxOccupancy: number }; auditLog: Array<{ actorId: string }> };
  };

  assert.equal(updated.status, 200);
  assert.equal(updatedBody.profile.coreRules.checkInTime, '16:00');
  assert.equal(updatedBody.profile.coreRules.maxOccupancy, 6);
  assert.equal(updatedBody.profile.auditLog.some((entry) => entry.actorId === 'host-owner'), true);
});
...
void test('stores explicit early-late pricing tiers and resolves deterministic exception policy', async () => {
  ...
  await patchPropertyBrainProfile(
    new Request(`http://localhost/api/command-center/property-brain/${encodeURIComponent(propertyId)}`, {
      ...
      body: JSON.stringify({
        ...
        earlyLatePolicy: {
          earlyCheckIn: { earliestTime: '12:00', latestTime: '15:30', priceTiers: [{ fromHour: 12, toHour: 14, amountUsd: 35 }] },
          lateCheckout: { earliestTime: '10:00', latestTime: '13:00', priceTiers: [{ fromHour: 11, toHour: 13, amountUsd: 45 }] }
        }
      })
    }),
    { params: Promise.resolve({ id: propertyId }) }
  ...
  const resolvedBody = (await resolved.json()) as { response: string; requiresClarification: boolean };
...
void test('stores cleaner contact preferences and required READY-ETA-NOT READY response format', async () => {
  ...
  const updateBody = (await update.json()) as { profile: { cleanerPreferences: { channel: string; requiredFormat: string } } };
...
void test('stores amenity caveats and safety details to avoid misinformation in amenity drafts', async () => {
    ...
    new Request(`http://localhost/api/command-center/property-brain/${encodeURIComponent(propertyId)}`, {
      ...
      body: JSON.stringify({
        ...
        amenityPolicies: {
          ...
          hotTub: { available: true, maxOccupancy: 4, safetyNotes: ['No glass', 'Shower before use'] },
  ...
  assert.equal(body.sources.some((source) => source.section === 'amenityPolicies.poolHeating'), true);
});
...
void test('returns clarification guidance when property brain context is incomplete', async () => {
  const response = await postPropertyBrainResolve(
    new Request('http://localhost/api/command-center/property-brain/resolve', {
      method: 'POST',
      headers: { 'content-type': 'application/json' },
      body: JSON.stringify({ propertyId: 'property:e09-missing', intent: 'early-check-in-request' })
    })
  );
  const body = (await response.json()) as { requiresClarification: boolean; missingFields: string[] };
  assert.equal(response.status, 200);
...
void test('returns property brain profile with completeness snapshot for operators', async () => {
  ...
  const response = await getPropertyBrainProfile(new Request(`http://localhost/api/command-center/property-brain/${propertyId}`), {
    params: Promise.resolve({ id: propertyId })
  });
  const body = (await response.json()) as { profile: { propertyId: string }; completeness: { coreRules: boolean } };
  assert.equal(response.status, 200);
...
void test('stores voice profile controls and applies them to resolved message style', async () => {
  ...
  const body = (await resolved.json()) as { response: string; styleApplied?: { tone: string; emojiUse: string } };
  assert.equal(resolved.status, 200);
...
void test('host operating profile tuning including economic sensitivity shifts guidance outcomes', async () => {
  await patchOperatingProfile(
    new Request('http://localhost/api/command-center/operating-profile', {
      method: 'PATCH',
      headers: { 'content-type': 'application/json' },
      body: JSON.stringify({ strictness: 70, generosity: 30, compensationCapUsd: 100, economicSensitivity: 90 })
    })
  );
  ...
  await patchOperatingProfile(
    new Request('http://localhost/api/command-center/operating-profile', {
      method: 'PATCH',
      headers: { 'content-type': 'application/json' },
      body: JSON.stringify({ strictness: 40, generosity: 65, compensationCapUsd: 300, economicSensitivity: 20 })
    })
  );
...
void test('computes readiness beyond cleaning using vendor conflicts maintenance and critical amenity signals', async () => {
  ...
  assert.equal(stateBody.state.readiness === 'blocked' || stateBody.state.readiness === 'at-risk', true);
...
void test('runs just-in-time checks for early-late requests with targeted intervention guidance', async () => {
  ...
  assert.equal(body.checks.some((check) => check.check === 'cleaner-readiness'), true);
});
...
void test('exposes always-on monitoring agent status with latest run metadata', async () => {
  ...
  const response = await getMonitoringAlerts(new Request('http://localhost/api/command-center/monitoring?includeAgentStatus=1'));
  const body = (await response.json()) as {
    items: unknown[];
...
void test('exposes approval queue as default landing screen metadata', async () => {
  ...
  const body = (await response.json()) as { defaultScreen: string; queueSummary: { total: number } };
  assert.equal(response.status, 200);
...
void test('supports urgency-sla sorting with quick preview-edit-send actions in queue payload', async () => {
  ...
  const response = await getQueueRoute(new Request('http://localhost/api/command-center/queue?sort=sla&includeUx=1'));
  const body = (await response.json()) as {
...
void test('returns conversation detail with source references intent labels and risk-trust badges', async () => {
  ...
  assert.equal(body.context.knowledgeSources.some((source) => Boolean(source.referenceUrl)), true);
});

void test('supports today priorities drilldowns to reservation and property details', async () => {
  const priorities = await getTodayPriorities(new Request('http://localhost/api/command-center/priorities?limit=1'));
  ...
  const drilldownBody = (await drilldown.json()) as {
    detail: { draftId: string; reservationId: string; propertyId: string; propertyState: { readiness: string } };
  };
...
void test('returns minimal property overview with readiness snapshot and blockers', async () => {
  ...
  const body = (await response.json()) as {
    items: Array<{ propertyId: string; readiness: string; blockers: string[]; pendingCount: number }>;
  };
  assert.equal(response.status, 200);
  assert.equal(body.items.length > 0, true);
...
void test('returns actionable error when hospitable outbound api is not configured', async () => {
  ...
  try {
    const response = await getHospitableMessages(
      new Request('http://localhost/api/integrations/hospitable/messages?reservationId=res-001&limit=5')
    );
    const body = (await response.json()) as { error?: string };
...
void test('fetches and normalizes hospitable outbound messages when api is configured', async () => {
  ...
  try {
    const response = await getHospitableMessages(
      new Request('http://localhost/api/integrations/hospitable/messages?reservationId=res-demo-001&limit=2')
    );
    const body = (await response.json()) as {
      source: string;
      count: number;
...
void test('supports single twilio ops number and cleaner 1-1 readiness threads', async () => {
  ...
  const list = await getTwilioThreads(new Request('http://localhost/api/integrations/twilio/threads'));
  ...
  assert.equal(body.threads.some((thread) => thread.cleanerId === 'cleaner-e14-001'), true);
});
...
void test('returns adoption metrics for command-center-first behavior and reduced anxiety trend', async () => {
  ...
  assert.equal(response.status, 200);
  assert.equal(body.adoption.commandCenterFirstRate >= 0 && body.adoption.commandCenterFirstRate <= 100, true);
  assert.equal(body.adoption.reducedAnxietyIndex >= 0 && body.adoption.reducedAnxietyIndex <= 100, true);
  assert.equal(body.adoption.workflowAdoptionRate >= 0 && body.adoption.workflowAdoptionRate <= 100, true);
